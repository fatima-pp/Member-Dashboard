...
class Zain_model extends CI_Model{
    ...
    public function execute_qry($qry){
        try{
            ...
            $this->save_log('get_client_info','error','error getting client info from zain_model',$error);
        } 
        catch(Exception $e){
    ...
    public function get_client_info($client_id = 0){
        ...
        if($client_id !== null && $client_id !== 0){
            ...
            $qry = $this->db->select('*')->from('clients')->where(array('id'=>$client_id,'status'=>$act_stat))->get();
            $response = $this->execute_qry($qry);
            return $response;
        }
        return false;
    }
